...
[![CI](https://github.com/Kantamaniprakash/Portfolio-Management-ML/actions/workflows/ci.yml/badge.svg)](https://github.com/Kantamaniprakash/Portfolio-Management-ML/actions/workflows/ci.yml)
...
> The news headlines used for the sentiment analysis are **SYNTHETIC**: they are generated deterministically from seeded templates in `src/data_collection.py`, with headline sentiment intentionally biased toward the actual next-day price direction. As a result, the **">60% directional accuracy" claim is validated on synthetic data, not on real financial news**, and should not be interpreted as evidence of real-world predictive performance. Price data (via `yfinance`) is real; the news corpus is not.
...
**What was measured, against what baseline, on what data:**

| Item | Setup |
|---|---|
| Data | Daily adjusted closes for the 8 tickers, 2019-01-01 → 2024-12-31 (`yfinance`), plus the **seeded synthetic news corpus** (`random_seed=42`, see disclosure above) |
| Signal metric | Next-day directional accuracy, evaluated on non-HOLD signals only |
| Accuracy baseline | 50% (random direction) |
| Portfolio baseline | Equal-weight buy-and-hold benchmark |
| Backtest assumptions | $100,000 initial capital, 0.1% cost per trade, daily close-to-close |

**Reported outcomes** (from the capstone run of `python main.py`; result artifacts are regenerated locally and not committed):

| Metric | Value |
|---|---|
| Overall directional accuracy | **>60%** — all 8 tickers above the threshold vs. the 50% random baseline |
| Sharpe ratio (strategy) | > 1.0 |
| Benchmark comparison | Positive excess return vs. equal-weight buy-and-hold |

To reproduce: run `python main.py` — per-ticker accuracy lands in `results/signal_accuracy.csv` and portfolio metrics in `results/strategy_metrics.csv`. Exact figures can shift slightly between runs as Yahoo Finance revises historical price data.

---

## Limitations

- **Synthetic news → accuracy by construction.** Headline sentiment is generated with a deliberate bias toward the actual next-day price direction (`src/data_collection.py`), so the NLP component's >60% accuracy validates the pipeline end-to-end — it is **not** evidence of a predictive edge on real financial news, which has never been evaluated here.
- **In-sample optimisation.** Markowitz weights are estimated on log returns from the full 2019–2024 sample and then used throughout the backtest of that same period — an implicit look-ahead that flatters backtest performance.
- **Optimistic execution model.** A flat 0.1% per-trade cost with same-day close-to-close fills; no slippage, market impact, or borrow costs are modelled, and SELL signals exit to cash rather than shorting.
- **Survivorship / selection bias.** The universe is 8 hand-picked US mega-caps that performed exceptionally over 2019–2024; results are unlikely to generalise to a broader or less fortunate universe.
- **Research-grade scope.** Single-machine batch pipeline with no live or paper trading, no walk-forward hyperparameter validation, and no committed result artifacts (everything under `results/` is regenerated per run).
...
```bash
git clone https://github.com/Kantamaniprakash/Portfolio-Management-ML.git
cd Portfolio-Management-ML
```

### Option A — uv (recommended, reproducible via committed lockfile)
