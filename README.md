...
### Validation
All 8 tickers surpass the 60% directional accuracy threshold — significantly above the 50% random baseline.

**Caveat:** this validation uses the **synthetic seeded news corpus** generated by `src/data_collection.py` (see the disclosure at the top of this README). Because headline sentiment is generated with a deliberate correlation to next-day returns, the NLP component's accuracy is by construction — the result demonstrates the pipeline's methodology end-to-end, not a genuine predictive edge on real news.
